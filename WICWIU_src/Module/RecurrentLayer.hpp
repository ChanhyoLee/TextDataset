...
public:
    ...
    */
    int Alloc(Operator<DTYPE> *pInput, int inputsize, int hiddensize, int outputsize, int use_bias, std::string pName) {
        ...
        //Tensorholder<DTYPE> *pWeight_h2h = new Tensorholder<DTYPE>(Tensor<DTYPE>::Random_normal(1, 1, 1, hiddensize, hiddensize+inputsize+1, 0.0, 0.01), "RecurrentLayer_pWeight_h2h_" + pName);    //bias 1개 일때!!!
        ...
        //매우매우 중요!!!!! cudnn 때문에 hidden2out, bias 부분 없애줌 -> BPTT에서 호출을 딱 한번만 해서!!!
