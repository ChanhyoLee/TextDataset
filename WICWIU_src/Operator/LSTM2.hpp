...
public:
  LSTM2(Operator<DTYPE> *pInput, Operator<DTYPE> *pWeightIG, Operator<DTYPE> *pWeightHG, Operator<DTYPE> *lstmBias)
    ...
    int BackPropagate(int pTime = 0) {
        ...
        for(int i=0; i<h; i++){
            (*OneGates)[Index5D(OneShape, pTime, 0, 0, 0, i)]    = (*tempForgetGates)[Index5D(EachShape, pTime, 0, 0, 0, i)];
            (*OneGates)[Index5D(OneShape, pTime, 0, 0, 0, h+i)]   = (*tempInputGates)[Index5D(EachShape, pTime, 0, 0, 0, i)];
            (*OneGates)[Index5D(OneShape, pTime, 0, 0, 0, 2*h+i)]   = (*tempCellGates)[Index5D(EachShape, pTime, 0, 0, 0, i)];
            (*OneGates)[Index5D(OneShape, pTime, 0, 0, 0, 3*h+i)] = (*tempOutputGates)[Index5D(EachShape, pTime, 0, 0, 0, i)];
        }
    ...
    int ForwardPropagateOnGPU(int pTime = 0) {
        ...
        ForgetGateSigmoid->ForwardPropagateOnGPU(pTime);
          ...
          std::cout<<ForgetGateSigmoid->GetResult()<<'\n';
        #endif
        ...
        InputGateSigmoid->ForwardPropagateOnGPU(pTime);
          ...
          std::cout<<InputGateSigmoid->GetResult()<<'\n';
        #endif
