...
public:
  GRU(Operator<DTYPE> *pInput, Operator<DTYPE> *pWeightIG, Operator<DTYPE> *pWeightHG, Operator<DTYPE> *pWeightICH, Operator<DTYPE> *pWeightHCH, Operator<DTYPE> *gBias, Operator<DTYPE> *chBias)
    ...
    int  ForwardPropagate(int pTime = 0) {
        ...
        for(int i=0; i<h; i++){
            (*tempRGates)[Index5D(EachShape, pTime, 0, 0, 0, i)]   = (*OneGates)[Index5D(OneShape, pTime, 0, 0, 0, i)];
            (*tempZGates)[Index5D(EachShape, pTime, 0, 0, 0, i)]   = (*OneGates)[Index5D(OneShape, pTime, 0, 0, 0, h+i)];
        }
    ...
    int BackPropagate(int pTime = 0) {
        ...
        for(int i=0; i<h; i++){
            (*OneGates)[Index5D(OneShape, pTime, 0, 0, 0, i)]    = (*tempRGates)[Index5D(EachShape, pTime, 0, 0, 0, i)];
            (*OneGates)[Index5D(OneShape, pTime, 0, 0, 0, h+i)]   = (*tempZGates)[Index5D(EachShape, pTime, 0, 0, 0, i)];
        }
