...
int main(int argc, char const *argv[]) {
    ...
    TextDataset<float> *dataset = new TextDataset<float>("Data/test80_1.txt", 100, ONEHOT);
    //batch 추가한 새로운 TextDataset!
    //BatchTextDataSet<float> *dataset = new BatchTextDataSet<float>("Data/test566.txt", BATCH, 100, ONEHOT);
    //CBOW를 위해 추가
    //WordTextDataSet<float> *dataset = new WordTextDataSet<float>("Data/test156.txt", 100, ONEHOT);
    //TextDataset2<float> *dataset = new TextDataset2<float>("Data/test566.txt", 200, ONEHOT);
    //TextDataset2<float> *dataset = new TextDataset2<float>("Data/test566.txt", 200, CBOWMODE);


    int Text_length = dataset->GetTextLength();     //나머지는 이거!!!
    //int Text_length = dataset->GetWordNum();        //TextDataset2일때만 이거!!!! 중요!!!
  ...
  //  NeuralNetwork<float> *net = new my_RNN(x_holder,label_holder, vocab_length);
    ...
    NeuralNetwork<float> *net = new my_SeqToSeq(x_holder,x_holder, label_holder, vocab_length);
